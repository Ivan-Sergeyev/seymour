...
A 2-sum $M = M_{\ell} \text{\;⊕₂\;} M_{r}$ merges two matroids that share exactly one common element. 
Conceptually, it glues $M_{\ell}$ and $M_{r}$ along that single element (analogous to joining two graphs at one edge) and then “removes” the identified element from the resulting matroid to avoid duplication.
