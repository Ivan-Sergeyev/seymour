...
A 3-sum $M = M_{\ell} \text{\;⊕₃\;} M_{r}$ joins two matroids that share a three-element structure (often called a “triangle”). 
In essence, $M_{\ell}$ and $M_{r}$ are fused together along this common triangle (a 3-element circuit/co-circuit) under certain compatibility conditions (notably an invertible $2\times 2$ submatrix in the overlap), and that substructure is then eliminated from the final matroid.
