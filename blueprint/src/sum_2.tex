...
\begin{definition}
    ...
    \uses{StandardRepr,def:two_sum_matrix}
    % \lean{}
    \leanok
...
\begin{lemma}
    ...
    \uses{def:two_sum_matrix,Matrix.IsTotallyUnimodular}
    % \lean{}
    \leanok
...
\begin{lemma}
    ...
    \uses{def:two_sum_matrix,Matrix.shortTableauPivot}
    % \lean{}
    \leanok
...
\begin{proof}
    \uses{def:two_sum_matrix,Matrix.shortTableauPivot,Matrix.shortTableauPivot_zero}
    \leanok
    Let
    \[
        B_{\ell}' = \begin{bmatrix} A_{\ell}' \\ r' \end{bmatrix}, \quad
...
\begin{proof}
    \uses{Matrix.isTotallyUnimodular_iff_forall_isPartiallyUnimodular,def:two_sum_matrix,lem:two_sum_bottom_tu,shortTableauPivot_submatrix_det_ni_signTypeCastRange,lem:two_sum_pivot}
    \leanok
    By Lemma~\ref{Matrix.isTotallyUnimodular_iff_forall_isPartiallyUnimodular}, it suffices to show that $B_{\ell} \oplus_{2, x, y} B_{r}$ is $k$-PU for every $k \in \mathbb{Z}_{\geq 1}$. We prove this claim by induction on $k$. The base case with $k = 1$ holds, since all entries of $B_{\ell} \oplus_{2, x, y} B_{r}$ are in $\{0, \pm 1\}$ by construction.
...
\begin{theorem}
    ...
    \uses{Matroid.IsRegular,def:two_sum_matroid}
    % \lean{}
    \leanok
...
\begin{proof}
    \uses{StandardRepr,def:two_sum_matroid,StandardRepr.toMatroid_isRegular_iff_hasTuSigning,lem:two_sum_tu,Matrix.IsTuSigningOf}
    \leanok
