...
def Matrix.IsPreTU (A : Matrix X Y R) (k : ℕ) : Prop :=
  ∀ f : Fin k → X, ∀ g : Fin k → Y, (A.submatrix f g).det ∈ SignType.cast.range

lemma exists_submatrix_of_not_isPreTU {A : Matrix X Y R} {k : ℕ} (hAk : ¬ A.IsPreTU k) :
    ∃ f : Fin k → X, ∃ g : Fin k → Y, (A.submatrix f g).det ∉ SignType.cast.range := by
  simpa [Matrix.IsPreTU] using hAk

lemma Matrix.isTotallyUnimodular_iff_forall_IsPreTU (A : Matrix X Y R) :
