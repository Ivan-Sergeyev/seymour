...
lemma VectorMatroid.exists_standardRepr_isBase [Field R] {G : Set α}
  ...
  · have trivi : (M.Aᵀ.submatrix hGY.elem id).range ⊆ M.Aᵀ.range
    · intro v ⟨j, hjv⟩
  ...
  have a4 : ∀ j, M.Aᵀ j ∈ Submodule.span R (M.Aᵀ.submatrix hGY.elem id).range := hRAGY ▸ a1

  -- have aa : (Set.range ((fun x => M.Aᵀ ↑x) : ↑(M.Y ↓∩ G) → ↑M.X → R)) ⊆ Submodule.span R (Set.range ((fun x => M.Aᵀ ↑x) : ↑(M.Y ↓∩ G) → ↑M.X → R)) := Submodule.subset_span
  -- -- have aaa : ∀
  ...
  ext I _hI
  · aesop
  have t : G ∪ M.Y \ G = M.Y := by exact Set.union_diff_cancel' (fun ⦃a⦄ a => a) hGY
  simp only [StandardRepr.toMatroid_indep_iff_elem', VectorMatroid.toMatroid_indep_iff_elem,
    Matrix.transpose_submatrix, t]
  have hI : I ⊆ M.Y := by exact subset_of_subset_of_eq _hI t
  simp only [hI, exists_true_left]
  ...
  -- have t1 := lin_indep.linearCombinationEquiv
  -- have t2 := lin_indep.repr
  have t5 := lin_indep.linearCombinationEquiv.toEquiv_injective
  classical
  have t4 : (C.submatrix id hYG.elem).prependId = C.submatrix id hGY.myEquiv := by
    ext i j
    cases j with
    | inl j1 =>
      simp
  ...
  -- let C' : (((C.submatrix id hYG.elem).prependIdᵀ ∘ Subtype.toSum) : (G ∪ (M.Y \ G)).Elem → G → R) =
...
-- good candidates of lemmas to use:
