...
@[app_unexpander List.seqApply]
private def List.seqApply_unexpand : Lean.PrettyPrinter.Unexpander
  | `($_ $l) => `($(l).$(Lean.mkIdent `seqApply))
  | _ => throw ()

omit [DecidableEq α] in
private lemma seqApply_empty (a : α) : [].seqApply a = a := by
  rfl
