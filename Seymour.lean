...
recall Matroid.Is1sumOf.pre {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → M.Is1sumOf Mₗ Mᵣ → M.IsPresumOf Mₗ Mᵣ
recall Matroid.Is1sumOf.hS {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → (hM : M.Is1sumOf Mₗ Mᵣ) → standardReprSum1 hM.hXY hM.hYX = hM.S
/--
info: Matroid.Is1sumOf.mk {α : Type} [DecidableEq α] {M Mₗ Mᵣ : Matroid α} (pre : M.IsPresumOf Mₗ Mᵣ)
  (hS : standardReprSum1 ⋯ ⋯ = some pre.S) : M.Is1sumOf Mₗ Mᵣ
-/
#guard_msgs in
#check Matroid.Is1sumOf.mk
...
recall Matroid.Is2sumOf.pre {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → M.Is2sumOf Mₗ Mᵣ → M.IsPresumOf Mₗ Mᵣ
recall Matroid.Is2sumOf.x {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → M.Is2sumOf Mₗ Mᵣ → α
recall Matroid.Is2sumOf.y {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → M.Is2sumOf Mₗ Mᵣ → α
recall Matroid.Is2sumOf.hx {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → (hM : M.Is2sumOf Mₗ Mᵣ) → hM.Sₗ.X ∩ hM.Sᵣ.X = {hM.x}
recall Matroid.Is2sumOf.hy {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → (hM : M.Is2sumOf Mₗ Mᵣ) → hM.Sₗ.Y ∩ hM.Sᵣ.Y = {hM.y}
recall Matroid.Is2sumOf.hS {α : Type} [DecidableEq α] : (M Mₗ Mᵣ : Matroid α) → (hM : M.Is2sumOf Mₗ Mᵣ) → standardReprSum2 hM.hx hM.hy hM.hXY hM.hYX = hM.S
/--
info: Matroid.Is2sumOf.mk {α : Type} [DecidableEq α] {M Mₗ Mᵣ : Matroid α} (pre : M.IsPresumOf Mₗ Mᵣ) {x y : α}
  (hx : pre.Sₗ.X ∩ pre.Sᵣ.X = {x}) (hy : pre.Sₗ.Y ∩ pre.Sᵣ.Y = {y}) (hS : standardReprSum2 hx hy ⋯ ⋯ = some pre.S) :
  M.Is2sumOf Mₗ Mᵣ
-/
#guard_msgs in
#check Matroid.Is2sumOf.mk
